...
##input parameters
OUTDIR <- "kallisto-sleuth_any-corfit"
RDATADIR <- "kallisto-sleuth_any-corfit/RData"
...
##read sleuth inputs
OUTPUT <- paste0(RDATADIR, "/sleuthObject.no_int.RData")
if(!file.exists(OUTPUT)){
    h5closeAll()
...
OUTPUT <- paste0(RDATADIR, "/obs_norm_filt_df.conds.no_int.RData")
if(!file.exists(OUTPUT)){
...
OUTPUT <- paste0(RDATADIR, "/obs_norm_filt2_df.conds.no_int.RData")
if(!file.exists(OUTPUT)){
...
OUTPUT <- paste0(RDATADIR, "/limma-voom.DGEList-corfit.no_int.RData")
if(!file.exists(OUTPUT)){
...
OUTPUT <- paste0(RDATADIR, "/limma-voom.contrastOut.no_int.RData")
if(!file.exists(OUTPUT)){
...
ggup <- plot_expression_so_tpm(so, genes=NO_vs_ANYup, mapgenes=rhtx2gene, conds, condcol="Int_Any", factors=c("NO", "YES"), tag="Int_Any, Up in NO")
ggdown <- plot_expression_so_tpm(so, genes=NO_vs_ANYdown, mapgenes=rhtx2gene, conds, condcol="Int_Any", factors=c("NO", "YES"), tag="Int_Any, Down in NO")
ggdownall <- plot_expression_so_tpm(so, genes=NO_vs_ANYdownall, mapgenes=rhtx2gene, conds, condcol="Int_Any", factors=c("NO", "YES"), tag="Int_Any, Down in NO")
